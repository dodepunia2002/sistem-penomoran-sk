...
const RiwayatAdmin = () => {
  ...
  const fetchData = async (search = '') => {
    try {
      ...
      const data = await apiCall(`/api/riwayat${params}`);
  ...
  const confirmDelete = async () => {
    setProcessing(true);
    try {
      await apiCall(`/api/riwayat/${deleteConfirm}`, { method: 'DELETE' });
  ...
  return (
    <div>
      <h2 className="page-title">RIWAYAT PENOMORAN SK</h2>
      
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <h3 style={{ fontSize: '1.1rem', textTransform: 'uppercase' }}>Data SK YG TELAH DIPROSES</h3>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state-text">{searchTerm ? 'Tidak ada data yang cocok.' : 'Belum ada riwayat penomoran.'}</p>
            <p className="empty-state-sub">{searchTerm ? 'Coba kata kunci lain.' : 'Data yang diterima akan muncul di sini.'}</p>
          </div>
        ) : (
              ...
              <tbody>
                {riwayat.map((row, idx) => (
                  <tr key={row.id} className="table-row-animate" style={{ animationDelay: `${idx * 0.05}s` }}>
                    ...
                    <td>{row.tanggal}</td>
                    <td><span className="sk-badge">{row.nomor_sk}</span></td>
                    <td style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
                      {editingId === row.id ? (
                        <>
                          ...
                          <X size={18} color="#9ca3af" style={{ cursor: 'pointer' }} onClick={cancelEdit} />
                        </>
                      ) : (
                        <>
        ...
        <div className="table-footer">Menampilkan {riwayat.length} dari {total} data</div>
      </div>
      ...
      {deleteConfirm && (
        <div className="modal-overlay" onClick={() => !processing && setDeleteConfirm(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="modal-title">Hapus Data?</h3>
            <p className="modal-text">Data ini akan dihapus secara permanen.</p>
